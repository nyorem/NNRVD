...
// @licend revoropt
#ifndef _REVOROPT_AABOX_HPP_
#define _REVOROPT_AABOX_HPP_
...
//initialize to a set of points
template< int Dim , typename Scalar >
void AABox<Dim,Scalar>::init_to( const Scalar* points, unsigned int size ) {
  init_to(points) ;
  if(size > 1) {
    fit_to(points + Dim, size - 1) ;
  }
}
...
//increase the box size to contain the given points
template< int Dim , typename Scalar >
void AABox<Dim,Scalar>::fit_to( const Scalar* points, unsigned int size ) {
...
template< int Dim , typename Scalar >
Scalar AABox<Dim,Scalar>::sq_distance_to( const Scalar point[Dim] ) const {
  //accumulator for the result
  Scalar res = 0 ;
  Scalar buf = 0 ;
  //test for every dimension
  for(int axis=0; axis<Dim; ++axis) {
    if(point[axis] < bounds_[2*axis]) {
      buf = bounds_[2*axis] - point[axis] ;
      res += buf*buf ;
    }
    if(point[axis] > bounds_[2*axis+1]) {
      buf = point[axis] - bounds_[2*axis+1] ;
      res += buf*buf ;
    }
  }

  if(res == 0) {
    res = bounds_[1] - bounds_[0] ;
    for(int axis=0; axis<Dim; ++axis) {
      const Scalar dmin = point[axis] - bounds_[2*axis] ;
      res = dmin < res ? dmin : res ;
      const Scalar dmax = bounds_[2*axis + 1] - point[axis] ;
      res = dmax < res ? dmax : res ;
    }
...
template< int Dim , typename Scalar >
Scalar AABox<Dim,Scalar>::distance_to( const Scalar point[Dim] ) const {
  Scalar d = sq_distance_to(point) ;
...
template< int Dim , typename Scalar >
bool AABox<Dim,Scalar>::clips_triangle_bbox( const Scalar* vertices
                                           ) const {
  //wrap th triangle as a mesh
  WrapMesh mesh(vertices, 3, single_triangle, no_neighbours, 1) ;
  return clips_triangle_bbox(0, &mesh) ;
}
...
template< int Dim , typename Scalar >
bool AABox<Dim,Scalar>::triangle_intersection( const Scalar* vertices,
                                               std::vector<Scalar>& output
                                             ) const {
  //wrap th triangle as a mesh
  WrapMesh mesh(vertices, 3, single_triangle, no_neighbours, 1) ;
...
template< int Dim , typename Scalar >
bool AABox<Dim,Scalar>::clips_triangle( const Scalar* vertices 
                                      ) const {
  //wrap th triangle as a mesh
  WrapMesh mesh(vertices, 3, single_triangle, no_neighbours, 1) ;
  return clips_triangle(0, &mesh);
