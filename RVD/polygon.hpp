...
class RVDVertex : public Eigen::Matrix<typename _Triangulation::Scalar, _Triangulation::VertexDim, 1> {
  ...
  public :
    ...
    typedef typename Eigen::Matrix<Scalar, Dim, 1> Vector ;
    ...
    RVDVertex( unsigned int x1_id, unsigned int x2_id, 
               ...
               const Triangulation* mesh, const Scalar* sites
             ) {
    ...
    void reset( unsigned int x1_id, unsigned int x2_id, unsigned int x3_id, 
      ...
      const Scalar c1 = (x2-x1).dot(v2-v1) ;
      const Scalar c2 = (x3-x1).dot(v2-v1) ;
      const Scalar c3 = 0.5*(v1+v2-2*x1).dot(v2-v1) ;
      const Scalar c4 = (x2-x1).dot(v3-v1) ;
      const Scalar c5 = (x3-x1).dot(v3-v1) ;
      const Scalar c6 = 0.5*(v1+v3-2*x1).dot(v3-v1) ;

      const Scalar det = (c1*c5-c2*c4) ;
#ifdef DEBUG
      if(det == 0) {
      ...
      const Scalar bary1 = denom*(c3*c5-c2*c6) ;
      const Scalar bary2 = denom*(c1*c6-c3*c4) ;
  ...
  private :
    ...
    void e_vertex_derivative( const int comb_index, 
      ...
      const Scalar k = 2*edge.dot(site_diff) ;
      const Scalar a = (v0+v1-2*x0).dot(site_diff)/k ;

      Eigen::Map< Eigen::Matrix<Scalar,Dim,Dim> > grad(output) ;
    ...
    void e_site_derivative( const int comb_index, 
      ...
      const Vector edge = x1-x0 ;
      const Vector site_diff = v1-v0 ;
      const Scalar half_k = edge.dot(site_diff) ;
    ...
    void f_vertex_derivative( const int comb_index, 
      ...
      //compute the gradient
      const Scalar c0 = (x0-x2).dot(v1-v0) ;
      const Scalar c1 = (x0-x2).dot(v2-v0) ;
      const Scalar c2 = (x2-x1).dot(v1-v0) ;
      const Scalar c3 = (x2-x1).dot(v2-v0) ;

      const Scalar b0 = (v0+v1-2*x2).dot(v1-v0) ;
      const Scalar b1 = (v0+v2-2*x2).dot(v2-v0) ;

      const Scalar quarter_k = (c0*c3-c2*c1) ;
      const Scalar a = (b0*c3-c2*b1)/(2*quarter_k) ;
    ...
    void f_site_derivative( const int comb_index, 
      ...
      //compute the gradient 
      const Scalar c0 = (x1-x0).dot(v0-v2) ;
      const Scalar c1 = (x2-x0).dot(v0-v2) ;
      const Scalar c2 = (x1-x0).dot(v2-v1) ;
      const Scalar c3 = (x2-x0).dot(v2-v1) ;
