...
#include <Tools/combinatorics.hpp>
#include <Tools/hash.hpp>

#include <Neighbours/lazy_nn.hpp>
...
class RVD {
  ...
  void set_default_clipper() {
    ...
    typedef NNClipper< Triangulation,LazyNN<Dim, Scalar> > DefaultClipper ; 
    //cleanup if necessary
    if(clipper_ != NULL && clipper_owned_) {
      delete clipper_ ;
    }
  ...
  const Scalar* sites() const { return clipper_->sites() ; }
...
class RVDStore {
  ...
  void operator()( unsigned int site, unsigned int triangle,
    ...
    for(unsigned int vertex = 0; vertex < size; ++vertex) {
      ...
      if( it == vmap.end() ) {
        ...
        vertex_combinatorics.push_back(v) ;
      } else {
  ...
  std::vector< RVDVertex<Triangulation> > vertex_combinatorics ;
} ;
