...
class NNClipper : public Clipper<_Triangulation> {
  ...
  Scalar max_neighbour_distance_ ;
  ...
  std::vector<Scalar> site_distances_ ;
  std::vector<Scalar> site_next_distances_ ;

  //distance between another site and the polygon vertices
  std::vector<Scalar> opposite_distances_ ;
  ...
  unsigned int neighbour_clip( RVDPolygon<Triangulation>& polygon,
    ...
    Scalar site_radius = *std::max_element( site_distances_.begin(), 
                                            site_distances_.end()
                                          ) ;
  ...
  Scalar compute_center_radius( RVDPolygon<Triangulation>& polygon, 
                                Vector& center,
                                Scalar site_radius
                              ) {
    Scalar center_radius = 0 ;
    for(unsigned int i=0; i<polygon.size(); ++i) {
  ...
  Scalar compute_finer_center_radius( RVDPolygon<Triangulation>& polygon, 
                                      Vector& center,
                                      std::vector<Scalar>& site_distances
                                    ) {
    Scalar center_radius = 0 ;
    for(unsigned int i=0; i<polygon.size(); ++i) {
      Vector d = (polygon[i].vertex - center) ;
  ...
  bool clip_by_plane( RVDPolygon<Triangulation>& polygon,
    ...
    for(unsigned int i = 0; i<size; ++i) {
      ...
      const Scalar my_side = opposite_distances_[i] - site_distances_[i] ;
      const Scalar next_side = opposite_distances_[(i+1)%size] 
